  ...
  <v-app id="app-inner">
    ...
    <v-toolbar color="indigo" dark fixed app>
      ...
      <span v-if="this.$store.state.auth.state === 'success'">
        <span>ID: {{this.$store.state.auth.username || ''}}</span>
        <v-btn @click="logout"><v-icon left>logout</v-icon> 退出登录</v-btn>
      </span>
      <span v-else>
        <v-btn @click="redirectLogin"><v-icon left>exit_to_app</v-icon> 登录</v-btn>
      </span>
    </v-toolbar>
    <v-content>
